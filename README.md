...
- 机体姿态：从侧栏进入近景页面，输入 β、滚转、俯仰和偏航角；支持左右/前后旋翼中心距与旋翼直径（mm），应用后更新模型，提供四个观察视角。两页共用紧凑机体模型，近景中的自定义尺寸仅作用于近景；飞行回放与规划使用默认尺寸。
...
飞行回放和机体近景共用按参考结构图制作的紧凑 H 型模型：中央双梁沿 X 连接左右两侧机臂，每侧机臂沿 Y 安装前后两个旋翼，通过短转轴和同步带与机架连接。β 使中央机架倾转，两侧机臂及 IMU 同步反向补偿，近景手动范围为 −90° 至 90°；连接件不会随角度伸缩。中央机架总深度不超过 32 mm，因此前后中心距必须比桨径至少大 40 mm，为完整旋翼扫掠范围留出每侧至少 4 mm 的间隙；左右中心距至少比桨径大 1 mm。另检查侧臂、电机和邻桨的实际网格与完整桨盘关系。整机姿态是共同刚体旋转，不改变内部间隙。

参数化飞行使用同一模型的完整包络计算穿缝角度与着陆高度，四个短 U 型电机底座共同接地，默认着陆中心高度约为 20.25 mm。原实验记录的中心高度会降到 0；回放显示仅在模型最低点进入地面时将模型抬至地表，不修改原始样本、曲线或导出数据。已经满足地面净距的规划轨迹无需此显示校正。
...
src/lib/droneDimensions.js      两页共用的默认米制尺寸
src/lib/sceneGeometry.js        回放显示的地面接触校正
...
src/lib/inspectionModel.js      两页共用的紧凑双梁、倾转机臂与旋翼细节
