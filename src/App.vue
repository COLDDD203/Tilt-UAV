  ...
  <div class="app-shell" :class="{ presenting }">
    ...
    <div class="main-shell">
      ...
      <main>
        ...
        <template v-if="page === 'workspace' || page === 'analysis'">
          ...
          <template v-if="page === 'workspace'">
            <div class="workspace-grid">
              <section class="panel flight-panel"><div class="panel-heading"><div class="panel-title"><Box :size="17"/><h2>三维飞行视图</h2><span class="subtle-tag">{{ stageName }}</span></div><div class="scene-actions"><div class="segmented camera-switch" aria-label="相机视角"><button v-for="c in [{ id: 'perspective', label: '透视' }, { id: 'top', label: '俯视' }, { id: 'side', label: '侧视' }]" :key="c.id" :class="{ selected: cameraMode === c.id }" @click="cameraMode = c.id">{{ c.label }}</button></div><button class="icon-button" title="重置视角" aria-label="重置视角" @click="fitKey++"><Focus :size="17"/></button><button class="icon-button" :title="presenting ? '退出专注模式' : '专注模式'" aria-label="切换专注模式" @click="presenting = !presenting"><Expand :size="16"/></button></div></div>
                <div class="scene-container"><DroneScene :sample="sample" :samples="samples" :show-trail="showTrail" :show-reference="showReference" :show-walls="showWalls" :camera-mode="cameraMode" :playing="playing" :fit-key="fitKey" :passage="scenePassage" :hover-target="hoverTarget"/><div class="scene-legend"><span><i class="legend-line"/>{{ isPlanned ? '规划航迹' : '飞行轨迹' }}</span><span v-if="hasReferences"><i class="legend-line dashed"/>参考轨迹</span></div></div>
                <div class="playback-bar"><div class="playback-top"><div class="playback-left"><button class="play-button" :aria-label="playing ? '暂停回放' : '开始回放'" @click="togglePlay"><Pause v-if="playing" :size="18" fill="currentColor"/><Play v-else :size="18" fill="currentColor"/></button><button class="icon-button" title="返回起点" aria-label="返回起点" @click="reset"><RotateCcw :size="17"/></button><div class="time-display">{{ f(currentTime, 2) }}<span>/ {{ f(endTime, 2) }} s</span></div><span class="playback-state">{{ playing ? '正在回放' : currentTime >= endTime ? '回放结束' : '已暂停' }}</span></div><div class="playback-right"><span>播放速度</span><ElSelect v-model="speed" size="small" aria-label="播放速度" class="speed-select"><ElOption v-for="rate in [.5, 1, 2, 4]" :key="rate" :label="`${rate}×`" :value="rate"/></ElSelect></div></div><div class="timeline"><ElSlider :model-value="currentTime" :min="startTime" :max="endTime" :step=".01" :format-tooltip="v => `${f(v, 2)} s`" aria-label="回放时间" @update:model-value="seek"/><div class="time-ticks"><span v-for="i in 5" :key="i">{{ f(startTime + (endTime - startTime) * (i - 1) / 4, 0) }} s</span></div></div></div>
              </section>
              <aside class="panel settings-panel">
                ...
                <div class="settings-body">
                  ...
                  <div class="attitude-box"><div class="section-label">姿态角</div><div class="attitude-values"><div><small>滚转</small><strong>{{ f(deg(sample.roll), 1) }}<em>°</em></strong></div><div><small>俯仰</small><strong>{{ f(deg(sample.pitch), 1) }}<em>°</em></strong></div><div><small>偏航</small><strong>{{ f(deg(sample.yaw), 1) }}<em>°</em></strong></div></div></div>
        ...
        <template v-else>
          <section class="guide-hero panel"><div class="guide-icon"><GraduationCap :size="32"/></div><div><span class="eyebrow">GETTING STARTED</span><h2>先复现仿真，再讲清楚你的实验。</h2><p>TiltLab 将现有 MATLAB / Simulink 结果转为可交互的三维回放。位置、姿态和曲线共享同一条时间轴，适合课堂讲解、项目答辩与研究记录。</p></div></section><div class="guide-grid"><article class="panel guide-card"><span class="guide-step">01</span><h2>运行与导出</h2><p>在 MATLAB 中完成仿真，将时间、位置、姿态和倾转角整理成等长列向量。使用项目内的 <code>matlab/export_demo_data.m</code> 导出标准 JSON。</p><div class="guide-tip">单位明确：位置用 m，时间用 s；输入角度统一用 rad 或 deg。</div></article><article class="panel guide-card"><span class="guide-step">02</span><h2>导入与检查</h2><p>点击“导入数据”，选择 JSON。系统检查字段、数值和时间顺序，再将角度统一换算。请先核对起点、终点和坐标轴方向。</p><div class="guide-tip">每次最多 100,000 点，35 MB；导入处理全程在浏览器内完成。</div></article><article class="panel guide-card"><span class="guide-step">03</span><h2>回放与分析</h2><p>播放或拖动时间轴观察机架倾转，切换视角与曲线类型。在“数据分析”页面查看全程指标，并导出完整 CSV。</p><div class="guide-tip"><kbd>Space</kbd> 播放 / 暂停　<kbd>←</kbd> <kbd>→</kbd> 前后 1 秒</div></article></div><section class="panel guide-details"><h2>参数化飞行怎么用</h2><p>在实验工作台直接输入狭缝净宽与最终悬停高度，再点击“计算并演示”。可在“更多参数”调整最大倾转角、每侧预留间隙与峰值速度。0.40 m 狭缝通常可以水平通过；0.24 m 狭缝配合默认余量，会自动计算出明显的倾转动作。</p><p>系统按固定机体和完整桨盘尺寸，在允许范围内以 0.25° 分辨率寻找最小可行倾转角。穿越期间机头沿通道对齐，宽度不够时提示无法通过。目标高度过高时，先在通道有效高度内穿越，再于出口外上升至目标并保持悬停。生成的方案可从“数据说明”导出 JSON，后续导入会同时恢复通道和任务阶段。</p><p>在页面顶部的“当前记录”中，可随时选择配套实验或已生成方案进行回放。　这些是浏览器生成的几何与运动学方案，不是论文控制器的闭环仿真。规划净距可直接观察，跟踪误差和旋翼转速不作虚构。</p></section><section class="panel guide-details"><h2>这个版本支持什么</h2><div class="scope-grid"><div><h3><Check :size="17"/>现在可以使用</h3><p>参数化狭缝与悬停高度、自动倾角计算、三维轨迹、曲线联动、实验导入与结果导出。构建后可在本地离线使用。</p></div><div><h3><Settings2 :size="17"/>后续可扩展</h3><p>网页调参运行 Simulink、故障注入与多算法对比需要接入实际可运行的模型及后端。参数化模式在浏览器中计算几何可行倾角与平滑航迹；真实闭环控制仿真仍需后端模型。</p></div></div><h3>坐标与模型约定</h3><p>世界坐标 Z 轴向上；姿态采用 Rz(yaw) · Ry(pitch) · Rx(roll)。机架在局部 Y 轴倾转，旋翼反向补偿。三维机体按示意比例绘制，参数化模式检查当前几何包络与规划通道的净距；结果不代表实机或动力学通过性验证。</p><ElButton @click="infoOpen = true"><Info :size="15"/>查看内置实验数据说明</ElButton></section>
        </template>
